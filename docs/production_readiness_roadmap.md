...
3. Для каждого публичного изменения обновлять:
   ...
   - API docs;
   - `handoff.md`.
4. Разбить монолитные тестовые файлы на логические единицы. Сейчас
   `lib/dlms-cosem/test/cosem/test_simple_objects.cpp` уже >12k строк —
   это перебор: ломает навигацию, замедляет инкрементальную сборку и
   делает code review болезненным. Правило на будущее:
   - один файл на один IC/класс: `test_cosem_<class>_object.cpp`
     (юнит-тесты конкретного `Cosem...Object`);
   - интеграционные сценарии — в отдельных файлах
     `test_<feature>_integration.cpp`, не смешивать с юнит-тестами;
   - регрессии — отдельные файлы `test_<class>_regression.cpp`,
     по одной регрессии на тест, со ссылкой на коммит/тикет;
   - общие фикстуры/хелперы — в `test/support/` (или аналогичном
     каталоге), без дубликата `BytesFromList`/`MakeName` в каждом
     файле.
   Миграция инкрементальная: при следующей правке любого `Cosem...`
   класса сначала вынести его блок тестов в отдельный файл, затем
   менять. Целевой порог — ни один файл тестов не превышает ~1500
   строк.
