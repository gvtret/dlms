...
TEST(AssociationClient, CallingApplicationTitleIsSentInAarq)
{
  const std::uint8_t title[] =
    {'C', 'L', 'I', 'T', 'I', 'T', 'L', 'E'};
  FakeApduChannel channel;
  ...
  dlms::association::AssociationOptions options =
    dlms::association::DefaultAssociationOptions();
  options.callingApplicationTitle.assign(title, title + sizeof(title));
  ...
  ASSERT_EQ(dlms::apdu::ApduStatus::Ok,
            dlms::apdu::DecodeAcseApdu(
              ...
              channel.sent.size(),
              sent));
  const std::vector<std::uint8_t> field = FieldBytes(sent.aarq, 0xA6);
  const std::uint8_t expected[] = {
    0xA6, 0x0A, 0x04, 0x08,
    'C', 'L', 'I', 'T', 'I', 'T', 'L', 'E'};
