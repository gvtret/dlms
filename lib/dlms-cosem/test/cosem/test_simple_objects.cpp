...
TEST(CosemDemandRegisterObject, MigratedToPerICFile)
{
  // IC 5 (Demand Register) coverage has moved to
  // test/cosem/test_cosem_demand_register_object.cpp per the
  // one-IC-one-file rule (P2.4). This placeholder keeps the gtest
  // suite name registered.
  SUCCEED();
}

TEST(CosemRegisterActivationObject, MigratedToPerICFile)
{
  // IC 6 (Register Activation) coverage has moved to
  // test/cosem/test_cosem_register_activation_object.cpp per the
  // one-IC-one-file rule (P2.4).
  SUCCEED();
}
...
// IC 70 (Disconnect Control) tests live in
// test_cosem_disconnect_control_object.cpp per rule P2.4.

} // namespace

TEST(CosemDisconnectControlObject, _MovedToPerIcFile)
{
  // See test_cosem_disconnect_control_object.cpp
  SUCCEED();
}
...
// IC 23 (IEC HDLC Setup) tests live in
// test_cosem_iec_hdlc_setup_object.cpp per rule P2.4
// (one test file per Interface Class).
TEST(CosemIecHdlcSetupObject, _MovedToPerIcFile) { SUCCEED(); }

// IC 61 (Register Table) tests live in test_cosem_register_table_object.cpp
// per rule P2.4 (one test file per Interface Class).

} // namespace

TEST(CosemRegisterTableObject, _MovedToPerIcFile)
{
  // See test_cosem_register_table_object.cpp
  SUCCEED();
}

TEST(CosemTcpUdpSetupObject, _MovedToPerIcFile)
{
  // See test_cosem_tcp_udp_setup_object.cpp
  SUCCEED();
}
...
// Helpers removed: typed IC 29 (Auto Connect) tests live in
// test_cosem_auto_connect_object.cpp.

} // namespace

// Legacy IC 29 (Auto Connect) opaque-buffer tests moved to
// test_cosem_auto_connect_object.cpp after the typed migration.
TEST(CosemAutoConnectObject, _MovedToPerIcFile)
{
  SUCCEED();
}

// Legacy IC 45 (GPRS Modem Setup) opaque-buffer tests moved to
// test_cosem_gprs_modem_setup_object.cpp after the typed migration.
TEST(CosemGprsModemSetupObject, _MovedToPerIcFile)
{
  SUCCEED();
}

// Legacy IC 28 (Auto Answer) opaque-buffer tests moved to
// test_cosem_auto_answer_object.cpp after the typed migration.
TEST(CosemAutoAnswerObject, _MovedToPerIcFile)
{
  SUCCEED();
}
...
TEST(CosemMacAddressSetupObject, _MovedToPerIcFile)
{
  // See test_cosem_mac_address_setup_object.cpp
  SUCCEED();
}
...
// IC 46 (SMTP Setup) typed tests moved to
// test/cosem/test_cosem_smtp_setup_object.cpp.
TEST(CosemSmtpSetupObject, _MovedToPerIcFile)
{
  SUCCEED();
}
...
TEST(CosemMBusSlavePortSetupObject, _MovedToPerIcFile)
{
  // Per-IC tests for IC 25 (M-Bus slave port setup).
  // See test_cosem_m_bus_slave_port_setup_object.cpp
  SUCCEED();
}
...
// IC 65 (Parameter Monitor) tests live in
// test_cosem_parameter_monitor_object.cpp per rule P2.4.

} // namespace

TEST(CosemParameterMonitorObject, _MovedToPerIcFile)
{
  // See test_cosem_parameter_monitor_object.cpp
  SUCCEED();
}

namespace {

// IC 62 (Compact Data) tests live in
// test_cosem_compact_data_object.cpp per rule P2.4.

} // namespace

TEST(CosemCompactDataObject, _MovedToPerIcFile)
{
  // See test_cosem_compact_data_object.cpp
  SUCCEED();
}
...
// IC 86 (PRIME PLC Application Identification) coverage moved to
// test/cosem/test_cosem_prime_plc_application_identification_object.cpp
TEST(CosemPrimePlcApplicationIdentificationObject,
     _MovedToPerIcFile)
{
  SUCCEED() << "see test_cosem_prime_plc_application_identification_object.cpp";
}
...
TEST(CosemSFskMacSyncTimeoutsObject, _MovedToPerIcFile)
{
  // See test_cosem_sfsk_mac_sync_timeouts_object.cpp
  SUCCEED();
}
...
// IC 58 tests moved to test_cosem_iso8802_llc_type2_setup_object.cpp;
// legacy `MakeIso8802LlcType2SetupObject` helper and the
// `Unsigned` / `LongUnsignedIc58` helpers were deleted with them.

} // namespace

TEST(CosemIso8802LlcType2SetupObject, _MovedToPerIcFile)
{
  // See test_cosem_iso8802_llc_type2_setup_object.cpp
  SUCCEED();
}
...
// IC 59 tests moved to test_cosem_iso8802_llc_type3_setup_object.cpp;
// legacy `MakeIso8802LlcType3SetupObject` helper and the
// `UnsignedIc59` / `LongUnsignedIc59` helpers were deleted with them.

} // namespace

TEST(CosemIso8802LlcType3SetupObject, _MovedToPerIcFile)
{
  // See test_cosem_iso8802_llc_type3_setup_object.cpp
  SUCCEED();
}

TEST(CosemWirelessModeQChannelObject, _MovedToPerIcFile)
{
  // Per-IC tests for IC 73 (Wireless Mode Q channel).
  // See test_cosem_wireless_mode_q_channel_object.cpp
  SUCCEED();
}
