...
## Phase 7. Server Association Processor

Deliverables:

- documentation-first contract for server-side association negotiation;
- `AssociationServerOptions` for server response capabilities;
- `AssociationServer` over caller-provided `IApduChannel`;
- one bounded AARQ receive -> AARE send accept operation;
- no-security LN only;
- deterministic rejection of authenticated AARQ input;
- fake-channel unit tests for success, receive/decode failure, unsupported
  authentication, and close behavior.

Commit message:

```text
feat(association): add server association processor
```
