...
- AARE responding AP title extraction for HLS GMAC server system title
  discovery.
- server-side HLS AARQ mechanism/challenge validation and AARE server
  challenge emission through a dedicated server strategy interface.
...
- endpoint-level wiring of the subsequent Association LN method calls.
...
Server-side HLS uses a separate non-owning strategy pointer because the server
must validate the selected AARQ mechanism and client challenge, then supply the
AARE server-to-client challenge. It still must not execute pass-3/pass-4.

```cpp
class IHighLevelSecurityServerStrategy
{
public:
  virtual AssociationStatus ValidateInitialChallenge(
    HighLevelSecurityMechanism mechanism,
    const std::vector<std::uint8_t>& clientChallenge) const = 0;
  virtual AssociationStatus BuildResponseChallenge(
    HighLevelSecurityMechanism mechanism,
    std::vector<std::uint8_t>& output) const = 0;
};
```

`AssociationServerOptions` shall carry a non-owning
`IHighLevelSecurityServerStrategy* highLevelSecurity` pointer. A null pointer
keeps current behavior: server HLS options return `UnsupportedAuthentication`.
...
Server-side HLS validation shall:

- require a non-null server HLS strategy;
- reject unknown mechanism ids and malformed ACSE authentication fields;
- extract the AARQ calling-authentication-value bytes exactly as encoded;
- call `ValidateInitialChallenge()` once with the selected mechanism and client
  challenge bytes;
- call `BuildResponseChallenge()` once after successful validation;
- reject empty or oversize server challenges while short-form BER fields are
  used.
...
Server-side `BuildAare()` shall add:

```text
8A 02 07 80
8B 07 60 85 74 05 08 02 <mechanism_id>
AA <challenge_size + 2> 80 <challenge_size> <server_challenge_bytes>
```

The AARE server challenge bytes are forwarded exactly as supplied by the server
strategy.
...
- server HLS accepts a matching mechanism and client challenge through the
  server strategy and sends AARE authentication fields;
- server HLS rejects missing strategy, unknown mechanism, malformed
  authentication fields, empty client challenge, strategy failure, and empty or
  oversize server challenge.
