...
The C API exposes client-side and server-side association lifecycles to C
callers without exposing C++ classes or ownership rules.

In scope:

- opaque `dlms_association_client_t` handle
- opaque `dlms_association_server_t` handle
...
- server `open`, `accept`, `release`, `close`, `is_associated`, and result
  copy
...
The C API creates association clients and servers from callbacks:
...
`dlms_association_server_get_result()` copies the same struct after successful
server accept.
