...
TEST(AssociationCApi, CallbackServerLifecycle)
{
  CallbackChannel channel;
  channel.open = false;
  channel.receiveStatus = DLMS_ASSOCIATION_STATUS_OK;
  channel.receives.push_back(MakeAarqBytes());
  channel.receives.push_back(MakeRlrqBytes());
  ...
  EXPECT_EQ(DLMS_ASSOCIATION_STATUS_OK,
            dlms_association_server_open(server));
  EXPECT_EQ(DLMS_ASSOCIATION_STATE_OPEN,
            dlms_association_server_get_state(server));
  EXPECT_EQ(DLMS_ASSOCIATION_STATUS_OK,
            dlms_association_accept(server));
  EXPECT_EQ(1, dlms_association_server_is_associated(server));

  dlms_association_result_t result;
  ASSERT_EQ(DLMS_ASSOCIATION_STATUS_OK,
            dlms_association_server_get_result(server, &result));
  EXPECT_EQ(6u, result.negotiated_dlms_version_number);
  EXPECT_NE(0u, result.server_max_receive_pdu_size);
  EXPECT_NE(0u, result.vaa_name);
...
TEST(AssociationCApi, RejectsMissingCallbacksAndNullHandles)
{
  ...
  EXPECT_EQ(DLMS_ASSOCIATION_STATUS_INVALID_ARGUMENT,
            dlms_association_server_release(nullptr));
  EXPECT_EQ(DLMS_ASSOCIATION_STATUS_INVALID_ARGUMENT,
            dlms_association_server_get_result(nullptr, nullptr));
  EXPECT_EQ(DLMS_ASSOCIATION_STATE_CLOSED,
            dlms_association_get_state(nullptr));
  EXPECT_EQ(DLMS_ASSOCIATION_STATE_CLOSED,
            dlms_association_server_get_state(nullptr));
