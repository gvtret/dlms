...
| Item                                                                                         | Tracked in                                                       |
| -------------------------------------------------------------------------------------------- | ---------------------------------------------------------------- |
| Server-side xDLMS trace emission through `XdlmsServerApduProcessor` (`SetTraceSink`/`TraceSink`) shipped in 0.101.0 and is reachable through `ServerEndpointOptions::xdlmsTraceSink` / `GatewayEndpointOptions::downstream.xdlmsTraceSink` (0.102.0). Since 0.103.0 every emitted event carries `conversationId = MakeConversationId(seedSource_->ConversationSeed(), invokeId)` (except pre-decode `SecurityFailed`/`DecodeFailed`, which still carry `kNoConversationId`), and the processor also pins the installed `IApduChannel*` via `SetCorrelation(conversationId)` so that downstream channel-layer trace events on the same response carry the same id. Endpoint composition still needs to wire `processor.SetApduChannel(...)` and `processor.SetConversationSeedSource(...)` so production builds (not just unit tests) see a non-zero id end-to-end. | Endpoint plumbing follow-up (separate minor). |
| Public server-dispatch trace sink `IServerDispatchTraceSink` (in `dlms-server`) and decorator `TracingXdlmsServerDispatcher` shipped in 0.102.0; reachable through `ServerEndpointOptions::serverDispatchTraceSink` / `GatewayEndpointOptions::downstream.serverDispatchTraceSink`. Dispatch-events still carry `conversationId == kNoConversationId` because the decorator does not yet read it back from the channel (`IApduChannel::CurrentConversationId()` is already published for this; only the decorator wiring is missing). | Dispatch-sink correlation + end-to-end integration test follow-up (separate minor). |
