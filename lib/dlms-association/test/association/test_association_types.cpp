...
TEST(AssociationTypes, DefaultOptionsUseDlmsInitiateDefaults)
{
  ...
  EXPECT_FALSE(options.hasProposedQualityOfService);
  EXPECT_EQ(0, options.proposedQualityOfService);
...
TEST(AssociationTypes, DefaultServerOptionsUseDlmsInitiateDefaults)
{
  const dlms::association::AssociationServerOptions options =
  ...
  EXPECT_EQ(dlms::association::AuthenticationMode::None,
            options.authenticationMode);
  EXPECT_TRUE(options.lowLevelSecurityCredential.empty());
  EXPECT_EQ(6u, options.negotiatedDlmsVersionNumber);
  EXPECT_EQ(0x00u, options.negotiatedConformance.bytes[0]);
  EXPECT_EQ(0x7eu, options.negotiatedConformance.bytes[1]);
  EXPECT_EQ(0x1fu, options.negotiatedConformance.bytes[2]);
  EXPECT_EQ(0x0200u, options.serverMaxReceivePduSize);
  EXPECT_EQ(0x0007u, options.vaaName);
}
