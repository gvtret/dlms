...
bool FindUnusedLoopbackPort(std::uint16_t& port)
{
  TestSocket probe = socket(AF_INET, SOCK_STREAM, IPPROTO_TCP);
  if (probe == kInvalidTestSocket) {
    return false;
  }
  ...
  socklen_t boundSize = sizeof(boundAddress);
#endif
  if (getsockname(probe, reinterpret_cast<sockaddr*>(&boundAddress), &boundSize) != 0) {
    CloseTestSocket(probe);
    return false;
  }
...
TEST(TcpStreamTransport, OpenUnusedLoopbackPortFails)
{
  TestSocketRuntime runtime;
  std::uint16_t unusedPort = 0;
  ASSERT_TRUE(FindUnusedLoopbackPort(unusedPort));

  TcpStreamTransportOptions options;
  options.host = "127.0.0.1";
  options.port = unusedPort;
  options.connectTimeout = TransportDuration{ 100 };
