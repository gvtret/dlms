...
struct dlms_association_server_t
{
  dlms::profile::IApduChannel* channel;
...
dlms::association::AssociationServerOptions ToCppServerOptions(
  const dlms_association_options_t* options)
{
  dlms::association::AssociationServerOptions cppOptions =
...
dlms_association_server_t* dlms_association_create_server_from_callbacks(
  const dlms_association_channel_callbacks_t* callbacks,
  const dlms_association_options_t* options)
{
  if (!ValidCallbacks(callbacks)) {
    ...
    std::unique_ptr<CApduChannelAdapter> channel(
      new CApduChannelAdapter(*callbacks,
                              options == 0
                                ? kDefaultReceiveBufferSize
                                : options->receive_buffer_size));
    std::unique_ptr<dlms::association::AssociationServer> server(
      new dlms::association::AssociationServer(
        *channel,
        ToCppServerOptions(options)));
    std::unique_ptr<dlms_association_server_t> handle(
      new dlms_association_server_t);
    handle->channel = channel.release();
    handle->server = server.release();
    return handle.release();
  } catch (...) {
    return 0;
  }
...
void dlms_association_destroy_server(
...
dlms_association_status_t dlms_association_server_open(
  dlms_association_server_t* server)
{
  if (server == 0 || server->server == 0) {
    return DLMS_ASSOCIATION_STATUS_INVALID_ARGUMENT;
  }

  try {
    return ToCStatus(server->server->Open());
  } catch (...) {
    return DLMS_ASSOCIATION_STATUS_INTERNAL_ERROR;
  }
}
...
dlms_association_status_t dlms_association_server_close(
  dlms_association_server_t* server)
{
  if (server == 0 || server->server == 0) {
    return DLMS_ASSOCIATION_STATUS_INVALID_ARGUMENT;
  ...
  try {
    return ToCStatus(server->server->Close());
  } catch (...) {
    return DLMS_ASSOCIATION_STATUS_INTERNAL_ERROR;
  }
}
...
dlms_association_status_t dlms_association_accept(
  dlms_association_server_t* server)
{
  if (server == 0 || server->server == 0) {
    return DLMS_ASSOCIATION_STATUS_INVALID_ARGUMENT;
  ...
  try {
    return ToCStatus(server->server->Accept());
  } catch (...) {
    return DLMS_ASSOCIATION_STATUS_INTERNAL_ERROR;
  }
}
...
dlms_association_status_t dlms_association_server_release(
  dlms_association_server_t* server)
{
  if (server == 0 || server->server == 0) {
    return DLMS_ASSOCIATION_STATUS_INVALID_ARGUMENT;
  ...
  try {
    return ToCStatus(server->server->Release());
  } catch (...) {
    return DLMS_ASSOCIATION_STATUS_INTERNAL_ERROR;
  }
}
...
dlms_association_state_t dlms_association_server_get_state(
  const dlms_association_server_t* server)
{
  ...
  try {
    return ToCState(server->server->State());
  } catch (...) {
    return DLMS_ASSOCIATION_STATE_CLOSED;
  }
}
...
int dlms_association_server_is_associated(
  const dlms_association_server_t* server)
{
  if (server == 0 || server->server == 0) {
    return 0;
  ...
  try {
    return server->server->IsAssociated() ? 1 : 0;
  } catch (...) {
    return 0;
  }
}
...
dlms_association_status_t dlms_association_server_get_result(
  const dlms_association_server_t* server,
  dlms_association_result_t* result)
{
  ...
  try {
    CopyResult(server->server->Result(), result);
    return DLMS_ASSOCIATION_STATUS_OK;
  } catch (...) {
    return DLMS_ASSOCIATION_STATUS_INTERNAL_ERROR;
  }
