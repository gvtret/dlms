...
{
public:
  virtual ~IHighLevelSecurityServerStrategy()
  {
  }

  virtual AssociationStatus SetCallingApplicationTitle(
...
struct AssociationOptions
{
  ...
  std::vector<std::uint8_t> callingApplicationTitle;
...
struct AssociationServerOptions
{
  ApplicationContext applicationContext;
  AuthenticationMode authenticationMode;
  std::vector<std::uint8_t> lowLevelSecurityCredential;
  const IHighLevelSecurityServerStrategy* highLevelSecurity;
